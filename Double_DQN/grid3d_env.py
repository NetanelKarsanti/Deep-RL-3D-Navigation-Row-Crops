...
class Grid3DEnv(gym.Env):
    ...
    def __init__(self, L=50, W=50, H=4, max_steps=200, render_mode=None, noise_prob=0.0, beta_z=1.0,
                 reward_mode="simple", air_cost=0.2, random_goal=False):
        super().__init__()
        assert render_mode in (None, *self.metadata["render_modes"])
        assert reward_mode in ("dist", "simple", "energy")
        ...
        self.beta_z    = beta_z
        self.d_xy_max  = float(np.sqrt((L - 1) ** 2 + (W - 1) ** 2))
    ...
    def _compute_reward(self, action, reached, moved):
        ...
        if self.reward_mode in ("simple", "energy"):
            # run_01 reward: discrete step costs; climbing (+z) costs more.
            base = -3.0 if action == 4 else -1.0
            # Avoidable-air penalty: horizontal cruise (±x, ±y) at altitude while
            # the z=0 cell directly below is free, i.e. the agent could have walked
            # on the ground. Ascend/descend and flight over an obstacle are exempt,
            # so a real obstacle crossing (climb -> cross -> descend) is never penalised.
            # "simple": flat penalty. "energy": graded by height (air_cost * z/(H-1)).
            if self.air_cost > 0.0 and action in HORIZONTAL_ACTIONS:
                ...
                if z >= 1 and not self.occupancy[x, y, 0]:
                    if self.reward_mode == "energy":
                        ...
                        base -= self.air_cost
            return base
        # "dist": distance-based shaped reward (run_03).
        x, y, z    = self.pos
        gx, gy     = self.goal[0], self.goal[1]
        dist_xy    = np.sqrt(float((gx - x) ** 2 + (gy - y) ** 2)) / self.d_xy_max
        z_norm     = z / (self.H - 1) if self.H > 1 else 0.0
        entry_cost = 2.0 if (action == 4 and z == 1) else 0.0
        return -dist_xy - self.beta_z * z_norm - entry_cost
